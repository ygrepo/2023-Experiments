...
# -- reparam_trick --
# see Kingma DP, Salimans T and Welling M. 2015. Variational 
# Dropout and the Local Reparameterization Trick. 
# Advances in Neural Information Processing Systems.
#  https://proceedings.neurips.cc/paper/2015/file/bc7316929fe1545bf0b98d114ee3ecb8-Paper.pdf
...
def Encoder(enc_input_dim, enc_output_dim, enc_hidden_dims):
    ...
    enc_hidden = [
        Dense(dim, activation="linear", name="enc_hidden_{}".format(i))
        for i, dim in enumerate(enc_hidden_dims)
    ]
...
def Decoder(dec_input_dim, dec_output_dim, dec_hidden_dims):
    ...
    dec_hidden = [
        Dense(dim, activation="linear", name="dec_hidden_{}".format(i))
        for i, dim in enumerate(dec_hidden_dims)
    ]
...
def Regressor(latent_dim, enc_input_dim, reg_hidden_dims):
    reg_latent_inputs = Input(
        shape=(latent_dim + enc_input_dim,), name="regressor_input"
    )
    ...
    reg_hidden = [
        Dense(dim, activation="linear", name="reg_hidden_{}".format(i))
        for i, dim in enumerate(reg_hidden_dims[1:])
    ]
