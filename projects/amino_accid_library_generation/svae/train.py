...
def train(
    ...
    # Keeps track of max difference in loss values between consecutive epochs,
    # across all training losses (model total, reconstruction, kl, regression)
    convergence_history = []

    # After convergence, model continues to train (stalls) for number of
    # epochs specified by 'patience' parameter
    stall = 0
    converged = False
    epochs_run = 0
    ...
    while not converged or stall < patience:
        ...
        for i in range(len(train_metrics)):
            # Adding a tf.keras.metrics.Mean() object for each tracked loss
            # that serves as a container for loss values per iteration
            # after each epoch, each loss container's state gets updated
            # to contain the most recent loss values, and
    ...
    # Make dataframes of train and val loss histories and combine
    # into in a single loss dataframe
