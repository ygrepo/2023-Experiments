...
def cluster_latent_space(
    ...
    if assay is not None:
        ...
        ax0.set_title("Latent Space | y_true", fontsize=9)
        ax1.set_title("Latent Space | y_pred", fontsize=9)
        ax2.set_title("Latent Space | cluster_mean", fontsize=9)

    if plot_name is None and assay is not None:
        assay_name = assay.replace(" ", "_").lower()
        plot_name = "{}_latent_space_clusters".format(assay_name)
    elif plot_name is None:
        plot_name = "latent_space_clusters"
    ...
    for cluster in top_clusters:
        ...
        for subcluster in np.argsort(-subcluster_assay_means):
            ...
            if assay is not None:
                ...
                print(
                    "\nSubcluster {}: mean = {}, ranking = {}".format(
                        subcluster,
                        subcluster_assay_means[subcluster],
                        subcluster_ranking[subcluster],
                    )
