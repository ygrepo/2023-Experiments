...
def loss(vae, X, Y, CV=0, alphabet=AAs, loss_weights=[0.2, 1.0, 0.2], kind="default"):
    ...
    if kind == "default":
        ...
        kl_loss = -0.5 * tf.reduce_mean(
            z_log_var
            - tf.square(z_mean - tf.reduce_mean(z_mean))
            - tf.exp(z_log_var)
            + 1
        )
    ...
    vae_loss = (
        loss_weights[0] * tf.cast(reconstruction_loss, tf.float64)
