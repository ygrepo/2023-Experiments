...
def prep_data(
    ...
    if target_assay_col is not None:
        df = df[df[target_assay_col] > target_threshold]
        print(df.shape)

    X, _ = seq_to_onehot(list(df[AA_colname].values), alphabet=alphabet)
    ...
    new_df["Y--{}".format(target_assay_col)] = Y.values
    ...
    train_df = (
        new_df[~new_df["test"]].copy().reset_index(drop=True).drop(columns="test")
    )
